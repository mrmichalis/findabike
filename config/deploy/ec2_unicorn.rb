...
case $1 in
  ...
  stop)
    ...
    then
      ...
      kill -s QUIT $pid;
      rm -f #{shared_path}/pids/unicorn.pid;
    else
  ...
  *)  
    ...
    ;;  
      ...
      File.open("./config/unicorn_ctl.sh", 'w') { |f| f.write(unicorn_ctl_content) }
      ...
      upload "./config/unicorn_ctl.sh",      "#{shared_path}/system/unicorn_ctl.sh"
      # chmod control script
      run "chmod 755 #{shared_path}/system/unicorn_ctl.sh"
    end
    ...
    task :update_unicorn_confs, :roles => :app do
    ...
    task :start_unicorn, :roles => :app do
      run "#{shared_path}/system/unicorn_ctl.sh start"
    end

    task :stop_unicorn, :roles => :app do
      run "#{shared_path}/system/unicorn_ctl.sh stop"
    end
    ...
    task :disable, :roles => :web do
      run "cp #{current_path}/public/system-origin/* #{current_path}/public/system/"
    end
